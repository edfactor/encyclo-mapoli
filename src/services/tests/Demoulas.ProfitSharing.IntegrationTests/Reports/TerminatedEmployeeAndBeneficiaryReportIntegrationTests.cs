...
public class TerminatedEmployeeAndBeneficiaryReportIntegrationTests : PristineBaseTest
{
    ...
    [Fact]
    public async Task CompareTotalsWithReady()
    {
        TestOutputHelper.WriteLine("🔍 COMPREHENSIVE COMPARISON: SMART vs READY TOTALS");
        TestOutputHelper.WriteLine("Comparing employee counts and key financial metrics.\n");

        // Use 2025 date range to match golden file
        DateOnly startDate = new DateOnly(2025, 01, 4);
        DateOnly endDate = new DateOnly(2025, 12, 27);
        ...
        // READY totals from golden file R3-QPAY066
        var readyEmployeeCount = 464;
        var readyAmountInProfitSharing = 24692640.86m;
        var readyVestedAmount = 22666201.39m;
        var readyTotalForfeitures = -9439.79m;
        var readyTotalBeneficiaryAllocations = 1349459.54m;

        // SMART totals
        var smartEmployeeCount = smartData.Response.Results.Count();
        var smartAmountInProfitSharing = smartData.TotalEndingBalance;
        var smartVestedAmount = smartData.TotalVested;
        var smartTotalForfeitures = smartData.TotalForfeit;
        var smartTotalBeneficiaryAllocations = smartData.TotalBeneficiaryAllocation;

        TestOutputHelper.WriteLine("📊 EMPLOYEE COUNT COMPARISON:");
        TestOutputHelper.WriteLine($"   READY: {readyEmployeeCount:N0} employees");
        TestOutputHelper.WriteLine($"   SMART: {smartEmployeeCount:N0} employees");
        TestOutputHelper.WriteLine($"   DIFFERENCE: {smartEmployeeCount - readyEmployeeCount:+#;-#;0} ({(smartEmployeeCount - readyEmployeeCount) / (double)readyEmployeeCount * 100:+0.##;-0.##;0.00}%)");
        TestOutputHelper.WriteLine("");

        TestOutputHelper.WriteLine("💰 AMOUNT IN PROFIT SHARING COMPARISON:");
        TestOutputHelper.WriteLine($"   READY: ${readyAmountInProfitSharing:N2}");
        TestOutputHelper.WriteLine($"   SMART: ${smartAmountInProfitSharing:N2}");
        TestOutputHelper.WriteLine($"   DIFFERENCE: ${smartAmountInProfitSharing - readyAmountInProfitSharing:+#,##0.00;-#,##0.00;0.00} ({(smartAmountInProfitSharing - readyAmountInProfitSharing) / readyAmountInProfitSharing * 100:+0.####;-0.####;0.0000}%)");
        TestOutputHelper.WriteLine("");

        TestOutputHelper.WriteLine("🏦 VESTED AMOUNT COMPARISON:");  
        TestOutputHelper.WriteLine($"   READY: ${readyVestedAmount:N2}");
        TestOutputHelper.WriteLine($"   SMART: ${smartVestedAmount:N2}");
        TestOutputHelper.WriteLine($"   DIFFERENCE: ${smartVestedAmount - readyVestedAmount:+#,##0.00;-#,##0.00;0.00} ({(smartVestedAmount - readyVestedAmount) / readyVestedAmount * 100:+0.####;-0.####;0.0000}%)");
        TestOutputHelper.WriteLine("");

        TestOutputHelper.WriteLine("📉 TOTAL FORFEITURES COMPARISON:");
        TestOutputHelper.WriteLine($"   READY: ${readyTotalForfeitures:N2}");
        TestOutputHelper.WriteLine($"   SMART: ${smartTotalForfeitures:N2}");
        TestOutputHelper.WriteLine($"   DIFFERENCE: ${smartTotalForfeitures - readyTotalForfeitures:+#,##0.00;-#,##0.00;0.00}");
        TestOutputHelper.WriteLine("");

        TestOutputHelper.WriteLine("👥 TOTAL BENEFICIARY ALLOCATIONS COMPARISON:");
        TestOutputHelper.WriteLine($"   READY: ${readyTotalBeneficiaryAllocations:N2}");
        TestOutputHelper.WriteLine($"   SMART: ${smartTotalBeneficiaryAllocations:N2}");
        TestOutputHelper.WriteLine($"   DIFFERENCE: ${smartTotalBeneficiaryAllocations - readyTotalBeneficiaryAllocations:+#,##0.00;-#,##0.00;0.00} ({(smartTotalBeneficiaryAllocations - readyTotalBeneficiaryAllocations) / readyTotalBeneficiaryAllocations * 100:+0.####;-0.####;0.0000}%)");
        TestOutputHelper.WriteLine("");

        // Calculate overall accuracy
        var employeeAccuracy = (1.0 - Math.Abs(smartEmployeeCount - readyEmployeeCount) / (double)readyEmployeeCount) * 100;
        var amountAccuracy = (1.0 - Math.Abs((double)(smartAmountInProfitSharing - readyAmountInProfitSharing)) / (double)readyAmountInProfitSharing) * 100;
        var vestedAccuracy = (1.0 - Math.Abs((double)(smartVestedAmount - readyVestedAmount)) / (double)readyVestedAmount) * 100;

        TestOutputHelper.WriteLine("🎯 OVERALL ACCURACY SUMMARY:");
        TestOutputHelper.WriteLine($"   Employee Count Accuracy: {employeeAccuracy:0.##}%");
        TestOutputHelper.WriteLine($"   Amount in Profit Sharing Accuracy: {amountAccuracy:0.####}%");
        TestOutputHelper.WriteLine($"   Vested Amount Accuracy: {vestedAccuracy:0.####}%");
        TestOutputHelper.WriteLine("");

        if (employeeAccuracy >= 95 && amountAccuracy >= 99.9)
        {
            TestOutputHelper.WriteLine("✅ EXCELLENT ALIGNMENT! Business logic is working correctly.");
        }
        else if (employeeAccuracy >= 90 && amountAccuracy >= 99.0)
        {
            TestOutputHelper.WriteLine("⚠️  GOOD ALIGNMENT with minor differences. Consider fine-tuning.");
        }
        else
        {
            TestOutputHelper.WriteLine("❌ SIGNIFICANT DIFFERENCES detected. Business logic review needed.");
        }

        // Assertion for test completion
        Assert.True(true, $"Comparison complete. Employee accuracy: {employeeAccuracy:0.##}%, Amount accuracy: {amountAccuracy:0.####}%");
    }
