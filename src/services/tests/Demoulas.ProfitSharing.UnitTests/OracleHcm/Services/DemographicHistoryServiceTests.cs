...
public class DemographicHistoryServiceTests
{
    ...
    [Fact]
    [Description("PS-XXXX : Change detection identifies all property changes")]
    public void PrepareUpdateExistingWithHistory_WithVariousPropertyChanges_DetectsAllChanges()
    {
        // Arrange
        var service = new DemographicHistoryService(_loggerMock.Object);
        ...
        var testCases = new List<(string PropertyName, Action<Demographic> Change)>
        {
            ("FirstName", d => d.ContactInfo.FirstName = "Changed"),
            ("LastName", d => d.ContactInfo.LastName = "Changed"),
            ("MiddleName", d => d.ContactInfo.MiddleName = "C"),
            ("PhoneNumber", d => d.ContactInfo.PhoneNumber = "999-999-9999"),
            ("EmailAddress", d => d.ContactInfo.EmailAddress = "new@email.com"),
            ("Street", d => d.Address.Street = "New Street"),
            ("City", d => d.Address.City = "New City"),
            ("State", d => d.Address.State = "NY"),
            ("PostalCode", d => d.Address.PostalCode = "99999"),
            ("BadgeNumber", d => d.BadgeNumber = 99999),
            ("GenderId", d => d.GenderId = 'F'),
            ("DateOfBirth", d => d.DateOfBirth = DateOnly.FromDateTime(DateTime.Now.AddYears(-30))),
            ("HireDate", d => d.HireDate = DateOnly.FromDateTime(DateTime.Now.AddYears(-5))),
            ("EmploymentStatusId", d => d.EmploymentStatusId = 't')
        };

        foreach (var (propertyName, change) in testCases)
        {
            var incoming = TestDataBuilder.CreateDemographic(oracleHcmId: existing.OracleHcmId);
            incoming.ContactInfo.FirstName = existing.ContactInfo.FirstName;
            incoming.ContactInfo.MiddleName = existing.ContactInfo.MiddleName;
            incoming.ContactInfo.LastName = existing.ContactInfo.LastName;
            incoming.ContactInfo.PhoneNumber = existing.ContactInfo.PhoneNumber;
            ...
            // Assert
            commands.Count.ShouldBe(2, $"Failed for property: {propertyName}");
            updatedCount.ShouldBe(1, $"Failed for property: {propertyName}");
        }
    }
}
