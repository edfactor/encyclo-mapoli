...
public class DemographicHistoryServiceTests
{
    ...
    [Fact]
    [Description("PS-XXXX : UpdateDemographic with ContactInfo change creates history with updated values")]
    public void PrepareUpdateExistingWithHistory_WithContactInfoChange_CreatesHistoryWithUpdatedValues()
    {
        // Arrange - employee changes their phone number and email
        var service = new DemographicHistoryService(_loggerMock.Object);

        var existing = TestDataBuilder.CreateDemographic(oracleHcmId: 100001);
        existing.Id = 1;
        existing.ContactInfo.FirstName = "John";
        existing.ContactInfo.LastName = "Smith";
        existing.ContactInfo.PhoneNumber = "555-1111";
        existing.ContactInfo.EmailAddress = "john.smith@old.com";
        existing.Address.Street = "123 Main St";
        existing.Address.City = "Boston";

        var incoming = TestDataBuilder.CreateDemographic(oracleHcmId: 100001);
        // Update contact info: phone and email changed, address changed
        incoming.ContactInfo.FirstName = "John";
        incoming.ContactInfo.LastName = "Smith";
        incoming.ContactInfo.PhoneNumber = "555-9999"; // Changed
        incoming.ContactInfo.EmailAddress = "john.smith@new.com"; // Changed
        incoming.Address.Street = "456 Oak Ave"; // Changed
        incoming.Address.City = "Cambridge"; // Changed
        ...
        // Assert
        commands.ShouldNotBeNull();
        commands.Count.ShouldBe(2); // UpdateDemographicCommand + AddHistoryCommand

        // Verify that the AddHistoryCommand captures the updated contact info
        var addHistoryCommand = commands.OfType<Data.Commands.Demographic.AddDemographicHistoryCommand>().FirstOrDefault();
        addHistoryCommand.ShouldNotBeNull();
        addHistoryCommand.DemographicHistory.ShouldNotBeNull();
        addHistoryCommand.DemographicHistory.PhoneNumber.ShouldBe("555-9999");
        addHistoryCommand.DemographicHistory.EmailAddress.ShouldBe("john.smith@new.com");
        addHistoryCommand.DemographicHistory.Street.ShouldBe("456 Oak Ave");
        addHistoryCommand.DemographicHistory.City.ShouldBe("Cambridge");
        updatedCount.ShouldBe(1);
    }

    [Fact]
    [Description("PS-XXXX : UpdateDemographic with Address change creates history with updated address")]
    public void PrepareUpdateExistingWithHistory_WithAddressChange_CreatesHistoryWithUpdatedAddress()
    {
        // Arrange - employee relocates
        var service = new DemographicHistoryService(_loggerMock.Object);

        var existing = TestDataBuilder.CreateDemographic(oracleHcmId: 100002);
        existing.Id = 2;
        existing.Address.Street = "100 Winter St";
        existing.Address.Street2 = "Suite 100";
        existing.Address.City = "Newton";
        existing.Address.State = "MA";
        existing.Address.PostalCode = "02165";

        var incoming = TestDataBuilder.CreateDemographic(oracleHcmId: 100002);
        // All address fields changed
        incoming.Address.Street = "200 Summer St";
        incoming.Address.Street2 = null; // Removed
        incoming.Address.City = "Waltham";
        incoming.Address.State = "MA";
        incoming.Address.PostalCode = "02451";
        ...
        var addHistoryCommand = commands.OfType<Data.Commands.Demographic.AddDemographicHistoryCommand>().FirstOrDefault();
        addHistoryCommand.ShouldNotBeNull();
        addHistoryCommand.DemographicHistory.Street.ShouldBe("200 Summer St");
        addHistoryCommand.DemographicHistory.Street2.ShouldBeNull();
        addHistoryCommand.DemographicHistory.City.ShouldBe("Waltham");
        addHistoryCommand.DemographicHistory.State.ShouldBe("MA");
        addHistoryCommand.DemographicHistory.PostalCode.ShouldBe("02451");
        updatedCount.ShouldBe(1);
    }
}
