...
internal sealed class ProfitDetailFaker: Faker<ProfitDetail>
{
    internal ProfitDetailFaker()
    {
        ...
        RuleFor(pd => pd.ProfitYear, fake => Convert.ToInt16(DateTime.Now.Year)).
            RuleFor(pd => pd.ProfitYearIteration, fake => (byte)0).
            RuleFor(pd => pd.ProfitClient, fake => (short)1).
            ...
            RuleFor(pd => pd.Forfeiture, fake => fake.Finance.Amount(0, 1000)).
            RuleFor(pd => pd.Month, fake => fake.Random.Byte(0, 12)).
            RuleFor(pd => pd.Year, fake => Convert.ToInt16(DateTime.Now.Year)).
            RuleFor(pd => pd.Comment, fake => fake.Lorem.Slug()).
            RuleFor(pd => pd.ZeroCont, fake => '0').
            RuleFor(pd => pd.FederalTaxes, fake => fake.Finance.Amount(0, 1000)).
            ...
            RuleFor(pd => pd.SSN, fake => DemographicFaker.ConvertSsnToLong(fake.Person.Ssn()));
    }
}
