...
/// </summary>
public sealed class PayServicesFullTimeStraightSalaryEndpoint : ProfitSharingEndpoint<PayServicesRequest, Results<Ok<PayServicesResponse>, NotFound, ProblemHttpResult>>
{
    private readonly IPayService _payService;
    private readonly ILogger<PayServicesFullTimeStraightSalaryEndpoint> _logger;

    public PayServicesFullTimeStraightSalaryEndpoint(
        IPayService payService,
        ILogger<PayServicesFullTimeStraightSalaryEndpoint> logger) : base(Navigation.Constants.QPAY600)
    {
    ...
    {
        Get("fulltimestraightsalary/{ProfitYear}");
        Group<PayServicesGroup>();
        Validator<PayServicesRequestValidator>();
        Summary(s =>
        {
            s.Description = "Retrieves Pay Services for full-time salaried employees.";
    ...
    public override Task<Results<Ok<PayServicesResponse>, NotFound, ProblemHttpResult>> ExecuteAsync(PayServicesRequest req, CancellationToken ct)
    {
        return this.ExecuteWithTelemetry(HttpContext, _logger, req, async () =>
        {
            ...
            var result = await _payService.GetPayServices(req, PayServicesRequest.Constants.FullTimeStraightSalary, ct);

            // Record business operation metrics
            EndpointTelemetry.BusinessOperationsTotal.Add(1,
                new("operation", "get-demographics-fulltimestraightsalary"),
                new("endpoint", nameof(PayServicesFullTimeStraightSalaryEndpoint)));
