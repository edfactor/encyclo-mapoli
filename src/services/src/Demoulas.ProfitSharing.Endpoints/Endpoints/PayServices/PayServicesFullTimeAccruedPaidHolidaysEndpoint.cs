...
/// </summary>
public sealed class PayServicesFullTimeAccruedPaidHolidaysEndpoint : ProfitSharingEndpoint<PayServicesRequest, Results<Ok<PayServicesResponse>, NotFound, ProblemHttpResult>>
{
    private readonly IPayService _payService;
    private readonly ILogger<PayServicesFullTimeAccruedPaidHolidaysEndpoint> _logger;

    public PayServicesFullTimeAccruedPaidHolidaysEndpoint(
        IPayService payService,
        ILogger<PayServicesFullTimeAccruedPaidHolidaysEndpoint> logger) : base(Navigation.Constants.QPAY600)
    {
    ...
    {
        Get("fulltimeaccruedpaidholidays/{ProfitYear}");
        Group<PayServicesGroup>();
        Validator<PayServicesRequestValidator>();
        Summary(s =>
        {
            s.Description = "Retrieves Pay Services for full-time hourly employees who accrue holidays.";
    ...
    public override Task<Results<Ok<PayServicesResponse>, NotFound, ProblemHttpResult>> ExecuteAsync(PayServicesRequest req, CancellationToken ct)
    {
        return this.ExecuteWithTelemetry(HttpContext, _logger, req, async () =>
        {
            ...
            var result = await _payService.GetPayServices(req, PayServicesRequest.Constants.FullTimeAccruedPaidHolidays, ct);

            // Record business operation metrics
            EndpointTelemetry.BusinessOperationsTotal.Add(1,
                new("operation", "get-pay-services-fulltimeaccruedpaidholidays"),
                new("endpoint", nameof(PayServicesFullTimeAccruedPaidHolidaysEndpoint)));
