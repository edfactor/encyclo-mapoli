...
/// </summary>
public sealed class PayServicesFullTimeEightPaidHolidaysEndpoint : ProfitSharingEndpoint<PayServicesRequest, Results<Ok<PayServicesResponse>, NotFound, ProblemHttpResult>>
{
    private readonly IPayService _payService;
    private readonly ILogger<PayServicesFullTimeEightPaidHolidaysEndpoint> _logger;

    public PayServicesFullTimeEightPaidHolidaysEndpoint(
        IPayService payService,
        ILogger<PayServicesFullTimeEightPaidHolidaysEndpoint> logger) : base(Navigation.Constants.QPAY600)
    {
    ...
    {
        Get("fulltimeeightpaidholidays/{ProfitYear}");
        Group<PayServicesGroup>();
        Validator<PayServicesRequestValidator>();
        Summary(s =>
        {
            s.Description = "Retrieves Pay Services for full-time hourly employees who get all holidays.";
    ...
    public override Task<Results<Ok<PayServicesResponse>, NotFound, ProblemHttpResult>> ExecuteAsync(PayServicesRequest req, CancellationToken ct)
    {
        return this.ExecuteWithTelemetry(HttpContext, _logger, req, async () =>
        {
            ...
            var result = await _payService.GetPayServices(req, PayServicesRequest.Constants.FullTimeEightPaidHolidays, ct);

            // Record business operation metrics
            EndpointTelemetry.BusinessOperationsTotal.Add(1,
                new("operation", "get-demographics-fulltimeeightpaidholidays"),
                new("endpoint", nameof(PayServicesFullTimeEightPaidHolidaysEndpoint)));
