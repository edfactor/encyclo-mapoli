...
public sealed class TerminatedEmployeesWithVestedBalanceBreakdownEndpoint : EndpointWithCsvBase<TerminatedEmployeesWithBalanceBreakdownRequest, MemberYearSummaryDto, TerminatedEmployeesWithVestedBalanceBreakdownEndpoint.BreakdownEndpointMap>
{
    ...
    private readonly ILogger<TerminatedEmployeesWithVestedBalanceBreakdownEndpoint> _logger;

    public TerminatedEmployeesWithVestedBalanceBreakdownEndpoint(IBreakdownService breakdownService, ILogger<TerminatedEmployeesWithVestedBalanceBreakdownEndpoint> logger)
    ...
    public override async Task<ReportResponseBase<MemberYearSummaryDto>> GetResponse(TerminatedEmployeesWithBalanceBreakdownRequest breakdownByStoreRequest, CancellationToken ct)
    {
        ...
        try
        {
            ...
            var result = await _breakdownService.GetTerminatedMembersWithVestedBalanceByStore(breakdownByStoreRequest, ct);

            // Record terminated employees with vested balance breakdown metrics
            EndpointTelemetry.BusinessOperationsTotal.Add(1,
                new("operation", "terminated-employees-vested-balance-breakdown"),
                new("endpoint", "TerminatedEmployeesWithVestedBalanceBreakdownEndpoint"),
                new("report_type", "breakdown"),
                new("employee_status", "terminated"),
                new("balance_filter", "vested-balance"));

            var resultCount = result?.Response?.Results?.Count() ?? 0;
            EndpointTelemetry.RecordCountsProcessed.Record(resultCount,
                new("record_type", "terminated-employees-vested-balance"),
                new("endpoint", "TerminatedEmployeesWithVestedBalanceBreakdownEndpoint"));

            _logger.LogInformation("Terminated employees with vested balance breakdown report generated, returned {Count} employees (correlation: {CorrelationId})",
