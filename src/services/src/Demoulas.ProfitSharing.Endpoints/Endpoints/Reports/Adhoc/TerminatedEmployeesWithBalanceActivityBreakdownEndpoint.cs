...
public sealed class TerminatedEmployeesWithBalanceActivityBreakdownEndpoint : EndpointWithCsvBase<TerminatedEmployeesWithBalanceBreakdownRequest, MemberYearSummaryDto, TerminatedEmployeesWithBalanceActivityBreakdownEndpoint.BreakdownEndpointMap>
{
    ...
    private readonly ILogger<TerminatedEmployeesWithBalanceActivityBreakdownEndpoint> _logger;

    public TerminatedEmployeesWithBalanceActivityBreakdownEndpoint(IBreakdownService breakdownService, ILogger<TerminatedEmployeesWithBalanceActivityBreakdownEndpoint> logger)
    ...
    public override async Task<ReportResponseBase<MemberYearSummaryDto>> GetResponse(TerminatedEmployeesWithBalanceBreakdownRequest breakdownByStoreRequest, CancellationToken ct)
    {
        ...
        try
        {
            ...
            var result = await _breakdownService.GetTerminatedMembersWithBalanceActivityByStore(breakdownByStoreRequest, ct);

            // Record terminated employees with balance activity breakdown metrics
            EndpointTelemetry.BusinessOperationsTotal.Add(1,
                new("operation", "terminated-employees-balance-activity-breakdown"),
                new("endpoint", "TerminatedEmployeesWithBalanceActivityBreakdownEndpoint"),
            ...
            EndpointTelemetry.RecordCountsProcessed.Record(resultCount,
                new("record_type", "terminated-employees-balance-active"),
                new("endpoint", "TerminatedEmployeesWithBalanceActivityBreakdownEndpoint"));

            _logger.LogInformation("Terminated employees with balance activity breakdown report generated, returned {Count} employees (correlation: {CorrelationId})",
