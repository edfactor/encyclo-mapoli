...
public sealed class YearEndServiceOld : IYearEndService
{
    ...
    public YearEndServiceOld(IProfitSharingDataContextFactory profitSharingDataContextFactory, ICalendarService calendar, IPayProfitUpdateService payProfitUpdateService,
    ...
    */
    public async Task RunFinalYearEndUpdatesAsync(short profitYear, bool rebuild, CancellationToken ct)
    {
        ...
        await _profitSharingDataContextFactory.UseWritableContext(async ctx =>
        {
            ...
            var frozenDemographicQuery = await _demographicReaderService.BuildDemographicQueryAsync(ctx, true);
            ...
            foreach (PayProfitDto employee in employees)
            {
                ...
                YearEndChange change = YearEndChangeCalculator.ComputeChange(profitYear, firstYearContribution, age, lastYearBalance, employee, fiscalEndDate, TimeProvider.System);
        ...
        CancellationToken ct)
    {
        ...
        var demographicQuery = await _demographicReaderService.BuildDemographicQueryAsync(ctx, useFrozenData: true);
