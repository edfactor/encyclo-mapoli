        ...
        CancellationToken cancellationToken = default)
    {
        var inquiryResults = await _dataContextFactory.UseReadOnlyContext(async ctx =>
        {
            ...
            var formattedQuery = combinedQuery.Select(x => new MasterInquiryResponseDto
            {
                ...
                Contribution = x.ProfitDetail.CalculateAmounts().contribution,
                Earnings = x.ProfitDetail.CalculateAmounts().earnings,
                Forfeiture = x.ProfitDetail.CalculateAmounts().forfeiture,
                Payment = x.ProfitDetail.CalculateAmounts().payment ,
