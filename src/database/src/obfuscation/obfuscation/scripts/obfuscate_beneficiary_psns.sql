...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE(chr(10) || '- - - Begin obfuscation of Beneficiary PSNs - - -');
 
    
    RESULT_CODE := OBFUSCATE_BENEFICIARY_PSNS;
    IF (RESULT_CODE <> 0) THEN
        raise_application_error( -20001, 'Error Obfuscating PSNs for Beneficiaries' );        
    END IF;
    DBMS_OUTPUT.PUT_LINE('...done.');
    COMMIT;
    
    DBMS_OUTPUT.PUT_LINE('- - - Completed obfuscation of Beneficiary PSNs - - -');
    ...
    dbms_output.put_line(chr(10) || '[TIME] Obfuscation of Beneficiary PSNs duration: ' 
...
EXCEPTION
        WHEN ex_custom THEN
            L_SQL_ERROR := SQLCODE;
            L_SQL_ERRM := SQLERRM;
            DBMS_OUTPUT.PUT_LINE('Execution of main beneficiary PSN script halted. ');
        ...
        WHEN OTHERS THEN
            ...
            DBMS_OUTPUT.PUT_LINE('There was an unhandled exeception in obfuscating the PSNs. ');
            DBMS_OUTPUT.PUT_LINE('Error number: '
                                 || L_SQL_ERROR
                                 || ' Message: '
                                 || L_SQL_ERRM);
            INSERT INTO log_messages (CALLER,  SQL_ERROR_CODE, ERROR_MESSAGE, CREATED_ON) 
            VALUES ('Main Ben PSN obfuscation ID script', L_SQL_ERROR, L_SQL_ERRM, sysdate);
END;