...
DECLARE
    ...
    CURSOR DELETE_PROFIT_DETAIL_SSNS_CUR IS
    SELECT
        PR_DET_S_SEC_NUMBER
    FROM
        PROFIT_DETAIL
    WHERE
        NOT EXISTS (
            ...
            FROM
                TARGET_EMP_SSN
            WHERE
                TARGET_EMP_SSN.SSN = PROFIT_DETAIL.PR_DET_S_SEC_NUMBER
        );
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('Preparing to delete PROFIT_DETAIL records. ');
    L_BATCH_NUMBER := 0;
    V_DONE := FALSE;
        ...
        WHERE
            NOT EXISTS (
                ...
                FROM
                    TARGET_ALL_SSN
                WHERE
                    TARGET_ALL_SSN.SSN = PROFIT_DETAIL.PR_DET_S_SEC_NUMBER
            );
    ...
    DBMS_OUTPUT.PUT_LINE('Delete count: ' || L_DELETE_COUNT);
    --COMMIT;
    ...
    dbms_output.put_line(chr(10) || '[TIME] Delete PROFIT_DETAIL records duration: ' 
...
EXCEPTION
        WHEN OTHERS THEN
            L_SQL_ERROR := SQLCODE;
            L_SQL_ERRM := SQLERRM;
            DBMS_OUTPUT.PUT_LINE('Script to delete unrelated PROFIT_DETAIL rows halted. ');
            DBMS_OUTPUT.PUT_LINE('Error number: '
                                 || L_SQL_ERROR
                                 || ' Message: '
                                 || L_SQL_ERRM);
            INSERT INTO log_messages (CALLER,  SQL_ERROR_CODE, ERROR_MESSAGE, CREATED_ON) 
            VALUES ('Error deleting unrelated PROFIT_DETAIL records', L_SQL_ERROR, L_SQL_ERRM, sysdate);
