...
SELECT COUNT(*)
  ...
  FROM (
	...
	INTO FAILED_STATEMENT
	FROM (
		SELECT d.SSN, pp.CURRENT_HOURS_YEAR, pp.CURRENT_INCOME_YEAR, pp.ETVA, pp.HOURS_EXECUTIVE, pp.INCOME_EXECUTIVE, pp.YEARS_IN_PLAN
		  FROM PAY_PROFIT pp 
		  JOIN DEMOGRAPHIC d ON pp.DEMOGRAPHIC_ID = d.ID
		 WHERE pp.PROFIT_YEAR = {CURRENT_YEAR}
	) spp
	FULL OUTER JOIN (
		SELECT p.PAYPROF_SSN ssn, p.PY_PH AS CURRENT_HOURS_YEAR, p.PY_PD AS CURRENT_INCOME_YEAR, p.PY_PS_ETVA AS ETVA, p.PY_PH_EXEC AS HOURS_EXECUTIVE, p.PY_PD_EXEC AS INCOME_EXECUTIVE,PY_PS_YEARS AS YEARS_IN_PLAN
    ...
    OR spp.YEARS_IN_PLAN <> rpp.YEARS_IN_PLAN;
