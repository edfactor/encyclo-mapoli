...
---TO "YOUR CURRENT SCHEMA" FROM - C##LEGACY 
-------------------------------------------------------------------------------------
DECLARE
...
--LOAD DEMOGRAPHIC TABLE TO - "YOUR CURRENT SCHEMA" FROM - C##LEGACY 
...
SELECT
    ...
    TO_NUMBER(SUBSTR( PYBEN.PYBEN_PSN, 0, 6)) AS BADGE_NUMBER,
     CASE
           WHEN d.ORACLE_HCM_ID IS NULL THEN d2.ORACLE_HCM_ID
           ELSE d.ORACLE_HCM_ID
           END                                  AS ORACLE_HCM_ID,
...
FROM C##LEGACY.PAYBEN PYBEN
INNER JOIN BENEFICIARY_CONTACT BC ON PYBEN.PYBEN_PAYSSN = BC.SSN
LEFT join DEMOGRAPHIC d on PYBEN.PYBEN_PAYSSN = d.SSN
LEFT join DEMOGRAPHIC d2 on TO_NUMBER(SUBSTR(PYBEN.PYBEN_PSN, 0, 6)) = d2.BADGE_NUMBER
LEFT JOIN C##LEGACY.PAYREL ON PYBEN.PYBEN_PSN = PAYREL.PYREL_PSN;
...
-- Migrate data into DISTRIBUTION_PAYEE table from C##LEGACY_PROFDIST
...
-- Migrate data into DISTRIBUTION_THIRDPARTY_PAYEE table from C##LEGACY_PROFDIST
...
--LOAD DISTRIBUTION TABLE TO - "YOUR CURRENT SCHEMA" FROM - C##LEGACY
...
SELECT
    ...
    END AS TRANSFER_PSN 
FROM C##LEGACY.PROFIT_SS_DETAIL;
---------------------------------------------------------------------------------------------------------------------
...
-- Step 2: Insert data from legacy table to new table
    ...
    (
        ...
        d.ORACLE_HCM_ID,  -- Lookup ORACLE_HCM_ID from DEMOGRAPHIC table using SSN
        psc.PAYABLE_NAME,
        psc.CHECK_AMOUNT,
        ...
        psc.PSC_CHECK_ID
    FROM
        C##LEGACY.PROFIT_SHARE_CHECKS psc
    JOIN
        DEMOGRAPHIC d ON psc.EMPLOYEE_SSN = d.SSN;  -- Lookup the ORACLE_HCM_ID using the SSN from DEMOGRAPHIC
