...
SELECT
    ...
    TO_NUMBER(SUBSTR( PYBEN.PYBEN_PSN, 0, 6)) AS EMPLOYEE_ID,
     CASE
           WHEN d.ORACLE_HCM_ID IS NULL THEN d2.ID
           ELSE d.ID
...
LEFT join DEMOGRAPHIC d2 on TO_NUMBER(SUBSTR(PYBEN.PYBEN_PSN, 0, 6)) = d2.EMPLOYEE_ID
...
SET COMMENT_TYPE_ID = 1,
    COMMENT_RELATED_ORACLE_HCM_ID = (SELECT ORACLE_HCM_ID FROM DEMOGRAPHIC d WHERE d.EMPLOYEE_ID = TO_NUMBER(SUBSTR(pd.REMARK,6,7))),
    COMMENT_RELATED_PSN_SUFFIX  = TO_NUMBER(SUBSTR(PD.REMARK,13,4))
WHERE REMARK LIKE 'XFER>%';

UPDATE PROFIT_DETAIL pd
SET COMMENT_TYPE_ID = 1,
    COMMENT_RELATED_ORACLE_HCM_ID = (SELECT ORACLE_HCM_ID FROM DEMOGRAPHIC d WHERE d.EMPLOYEE_ID = TO_NUMBER(SUBSTR(pd.REMARK,7,7))),
