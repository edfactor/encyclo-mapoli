...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE(chr(10) || '- - - Begin obfuscation of SSNs - - -');
    
    DBMS_OUTPUT.PUT_LINE('Obfuscating employee SSNs...');
    RESULT_CODE := OBFUSCATE_EMPLOYEE_SSNS;
    IF (RESULT_CODE <> 0) THEN
        
        raise_application_error( -20001, 'Error Obfuscating SSNs for Employees' );        
    END IF;
    DBMS_OUTPUT.PUT_LINE('...done.');
    COMMIT;    
    
    DBMS_OUTPUT.PUT_LINE('- - - Completed obfuscation of Employee SSNs - - -');
    ...
    dbms_output.put_line(chr(10) || '[TIME] Obfuscation of Employee SSNs duration: ' 
...
EXCEPTION
        WHEN ex_custom THEN
            L_SQL_ERROR := SQLCODE;
            L_SQL_ERRM := SQLERRM;
            DBMS_OUTPUT.PUT_LINE('Execution of main employee obfuscation SSN script halted. ');
            DBMS_OUTPUT.PUT_LINE('Error number: '
                                 || L_SQL_ERROR
                                 || ' Message: '
                                 || L_SQL_ERRM);
            INSERT INTO log_messages (CALLER,  SQL_ERROR_CODE, ERROR_MESSAGE, CREATED_ON) 
            VALUES ('Main SSN obfuscation script', L_SQL_ERROR, L_SQL_ERRM, sysdate);
        WHEN OTHERS THEN
            L_SQL_ERROR := SQLCODE;
            L_SQL_ERRM := SQLERRM;
            ...
            DBMS_OUTPUT.PUT_LINE('There was an unhandled exeception in obfuscating emp SSNs. ');
            DBMS_OUTPUT.PUT_LINE('Error number: '
                                 || L_SQL_ERROR
                                 || ' Message: '
                                 || L_SQL_ERRM);
            INSERT INTO log_messages (CALLER,  SQL_ERROR_CODE, ERROR_MESSAGE, CREATED_ON) 
            VALUES ('Main employee SSN obfuscation ID script', L_SQL_ERROR, L_SQL_ERRM, sysdate);
END;