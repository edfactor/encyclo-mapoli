...
describe("useDecemberFlowProfitYear", () => {
  const createMockStore = (selectedProfitYear: number) => {
    return configureStore({
      reducer: {
        yearsEnd: () => ({
          selectedProfitYearForDecemberActivities: selectedProfitYear
        })
      }
    });
  };
  ...
  it("should return the selected profit year from Redux store", () => {
    ...
    const { result } = renderHook(() => useDecemberFlowProfitYear(), { wrapper: wrapper(mockStore) });
  ...
  it("should return different profit years based on store state", () => {
    ...
    const { result: result2023 } = renderHook(() => useDecemberFlowProfitYear(), {
    ...
    const { result: result2025 } = renderHook(() => useDecemberFlowProfitYear(), {
