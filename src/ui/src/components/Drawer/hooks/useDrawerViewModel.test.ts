...
import { NavigationResponseDto, NavigationDto } from "../../../../reduxstore/types";
...
describe("useDrawerViewModel", () => {
  ...
  describe("Initial State", () => {
    it("should initialize with drawer closed", () => {
      ...
      const { result } = renderHook(
        () => useDrawerViewModel(mockNavigationData, mockDrawerConfig),
  ...
  describe("Edge Cases", () => {
    it("should handle undefined navigation data gracefully", () => {
      const store = createMockStore();
      const { result } = renderHook(
        () => useDrawerViewModel(undefined, mockDrawerConfig),
        { wrapper: createWrapper(store) }
      );
    ...
    it("should handle empty navigation data", () => {
      ...
      const store = createMockStore();
      const { result } = renderHook(
        () => useDrawerViewModel(emptyData, mockDrawerConfig),
        { wrapper: createWrapper(store) }
      );
    ...
    it("should filter non-navigable items", () => {
      ...
      const store = createMockStore();
      const { result } = renderHook(
        () => useDrawerViewModel(dataWithHidden, mockDrawerConfig),
        { wrapper: createWrapper(store) }
      );
