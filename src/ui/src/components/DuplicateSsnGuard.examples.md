...
<DuplicateSsnGuard mode="warning">
  {({ prerequisitesComplete, hasDuplicates }) => (
    <>
      ...
      />
      {hasDuplicates && <InfoMessage>Note: Results may be affected by duplicate SSNs in the system.</InfoMessage>}
    </>
  )}
</DuplicateSsnGuard>;
```
  ...
  return (
    <DuplicateSsnGuard mode={guardMode}>
      {({ prerequisitesComplete, hasDuplicates, refresh }) => (
        <div>
          {hasDuplicates && userRole === "ADMINISTRATOR" && <button onClick={refresh}>Recheck for Duplicates</button>}
          <SearchAndReset
            handleReset={handleReset}
            handleSearch={validateAndSearch}
...
<DuplicateSsnGuard>
  {({ prerequisitesComplete }) => <Button disabled={!prerequisitesComplete}>Submit</Button>}
</DuplicateSsnGuard>
```
...
<DuplicateSsnGuard mode="warning">
  {({ hasDuplicates }) => (
    <div>
      ...
      {hasDuplicates && <Typography color="warning.main">⚠️ Some results may be affected by duplicate SSNs</Typography>}
    </div>
  )}
</DuplicateSsnGuard>
```
