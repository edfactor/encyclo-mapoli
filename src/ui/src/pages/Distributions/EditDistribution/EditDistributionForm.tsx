...
export interface EditDistributionFormRef {
  submit: () => void;
  reset: () => void;
  isFormValid: () => boolean;
...
interface EditDistributionFormProps {
  ...
  onSubmit: (request: EditDistributionRequest) => Promise<void>;
  onReset: () => void;
...
// Validation schema (same as AddDistributionForm)
const schema = yup.object().shape({
  ...
  memo: yup.string().max(500, "Memo cannot exceed 500 characters").optional(),
  thirdPartySsn: yup
    .string()
    .test("valid-ssn", "SSN must be 9 digits", (value) => {
  ...
  ({ distribution, stateTaxRate, onSubmit, age, vestedAmount }, ref) => {
    const {
      ...
      defaultValues: {
        ...
        taxCode: distribution.taxCodeId || "",
        reasonCode: distribution.frequencyId || "",
        amountRequested: distribution.grossAmount?.toString() || "0.00",
        fedTaxOverride: false,
        fedTaxPct: "20.0",
        fedTax: distribution.federalTax?.toString() || "0.00",
        stateTaxOverride: false,
        stateTaxPct: stateTaxRate?.toFixed(2) || "0.00",
        stateTax: distribution.stateTax?.toString() || "0.00",
        sequenceNumber: distribution.paymentSequence?.toString() || "1",
        memo: "",
    ...
    // Auto-set tax code when 3rd party name is entered
    useEffect(() => {
    ...
    // Clear tax overrides when Rollover (R) is selected
    useEffect(() => {
      if (reasonCode === "R") {
        setValue("fedTaxOverride", false);
        setValue("stateTaxOverride", false);
    ...
    const handleFormSubmit = async (data: EditDistributionFormData) => {
      // Build EditDistributionRequest
      const request: EditDistributionRequest = {
        id: distribution.id,
        badgeNumber: distribution.badgeNumber || 0,
        statusId: data.paymentFlag || "C",
        frequencyId: data.reasonCode || "M",
        payeeId: null,
            ...
            <Controller
              ...
              render={({ field }) => (
                <TextField
                  ...
                  <MenuItem value="P">P - Pay Direct</MenuItem>
                  <MenuItem value="R">R - Rollover</MenuItem>
                  <MenuItem value="M">M - Monthly</MenuItem>
                  <MenuItem value="Q">Q - Quarterly</MenuItem>
                  <MenuItem value="A">A - Annually</MenuItem>
                </TextField>
              )}
            />
          </Grid>
...
EditDistributionForm.displayName = "EditDistributionForm";
