...
interface TermedNoPriorGridProps {
  pageNumberReset: boolean;
  setPageNumberReset: (reset: boolean) => void;
}

const TermedNoPriorGrid: React.FC<TermedNoPriorGridProps> = ({ pageNumberReset, setPageNumberReset }) => {
    ...
    <>
      <div style={{ padding: "0 24px 0 24px" }}>
        <Typography
          variant="h2"
          sx={{ color: "#0258A5" }}>
          {`TERMED NO PRIOR PS REPORT (${data?.response?.total || 0} records)`}
        </Typography>
      </div>
      <DSMGrid
        preferenceKey={CAPTIONS.PAY426_TERMINATED_NO_PRIOR}
