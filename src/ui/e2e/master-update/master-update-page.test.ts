...
test.describe("Master Update page: ", () => {
  ...
  test("click Preview without changing fields triggers backend and returns 200", async ({ page }) => {
    ...
    try {
      const response = await page.waitForResponse(
        (resp) => resp.url().includes("yearend/profit-sharing-update") || resp.url().includes("profit-sharing-update"),
        { timeout: 5000 }
      );
  ...
  test("pagination works on result grid", async ({ page }) => {
    ...
    await preview.click();
    try {
      const resp = await page.waitForResponse((r) => r.url().includes("yearend/profit-sharing-update"), {
        timeout: 5000
      });
    ...
    if (!isNextDisabled) {
      await nextBtn.click();
      try {
        const response1 = await page.waitForResponse((r) => r.url().includes("yearend/profit-sharing-update"), {
          timeout: 5000
        });
