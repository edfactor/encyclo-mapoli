...
test.describe("Military Contributions: ", () => {
  ...
  test("add badge number and click on search", async ({ page }) => {
    ...
    const [response] = await Promise.all([page.waitForResponse((resp) => resp.url().includes("military"))]);
    await expect(response.status()).toBe(200);
  });
  ...
  test("ADD MILITARY CONTRIBUTION button click", async ({ page }) => {
    ...
    const [response1] = await Promise.all([
      page.waitForResponse((resp) => resp.url().includes("military") && resp.request().method() === "POST")
    ]);
