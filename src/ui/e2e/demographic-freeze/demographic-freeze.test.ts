...
test.describe("Demographic Freeze: ", () => {
    ...
    test('filling As of Date greater than current date', async ({ page }) => {
        ...
        await page.getByRole('textbox', { name: 'MM/DD/YYYY' })
            .fill(`${currentMonth}/${currentDay}/${currentYear}`);

        await page.locator('#asOfTime').fill(formattedTime);

        await expect(page.getByRole('button', { name: 'CREATE FREEZE POINT' }))
            .toBeDisabled();
    });
    ...
    test('add values and click on CREATE FREEZE POINT', async ({ page }) => {
        ...
        const [response] = await Promise.all([page.waitForResponse((resp) =>
            resp.url().includes('itdevops/freeze'))]);
        await expect(response.status()).toBe(200);
    });
