    ...
    cellRenderer: (params: ICellRendererParams) => {
      ...
      if (params.data?.isDetail) {
        const hideInDetails = !detailColumns.some(
          (detailCol) => detailCol.field === (column as ColDef).field
        );
        if (hideInDetails) {
...
const detailOnlyColumns = detailColumns
  .filter((detailCol) =>
    !mainColumns.some((mainCol) => mainCol.field === detailCol.field)
  )
  .map((column) => ({
    ...column,
    cellRenderer: (params) => {
...
const [editedValues, setEditedValues] = useState<
  Record<string, { value?: number; hasError?: boolean }>
>({});
...
const handleBulkSave = async (
  ...
  try {
    ...
    await Promise.all(
      requests.map((req) => saveAdjustmentMutation(req).unwrap())
    );
...
const detailColumns = useMemo(
  ...
  [addRowToSelection, removeRowFromSelection, selectedProfitYear,
   handleSave, handleBulkSave, isReadOnly]
);
```
