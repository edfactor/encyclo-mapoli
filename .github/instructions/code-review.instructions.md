...
## Accessibility review checklist (WCAG 2.2 AA - UI changes only)

For UI components in `src/ui/src/`, reviewers MUST verify:

### Form Inputs & Labels

- All inputs have unique `id` attributes
- All `FormLabel` components have matching `htmlFor` attributes
- Required inputs have `aria-required="true"`
- Inputs in error state have `aria-invalid={true}`
- All inputs have `aria-describedby` linking to error/hint text

### Error Handling

- Inline validation errors use `aria-live="polite"`
- Form submission errors use `role="alert"` with `aria-live="assertive"`
- Error messages are programmatically associated with inputs via `aria-describedby`

### Loading States

- Async operations include `LoadingAnnouncement` component for screen readers
- Visual loading indicators (e.g., `CircularProgress`) have `aria-hidden="true"`

### Buttons & Interactive Elements

- Icon-only buttons have descriptive `aria-label` attributes
- All interactive elements are keyboard accessible (Tab, Enter, Space, Escape)
- Loading buttons have `aria-busy` attribute when processing

### Modals & Dialogs

- Modals have `aria-labelledby` pointing to title
- Modals have `aria-describedby` pointing to description (if present)
- First input receives focus on modal open (`autoFocus`)

### Input Formatting (where applicable)

- SSN inputs use `formatSSNInput` for live formatting
- Phone inputs use `formatPhoneInput` for live formatting
- Zip code inputs use `formatZipCode` for MA zip auto-formatting
- All formatted inputs include `aria-describedby` with format hints

### Placeholders

- Form inputs have helpful placeholders showing expected format
- Placeholders use centralized `INPUT_PLACEHOLDERS` constants

### Color Contrast

- Text meets 4.5:1 contrast ratio (normal text)
- UI components meet 3:1 contrast ratio (buttons, borders)
- Focus indicators meet 3:1 contrast ratio

### Automated Testing

- Component unit tests include `jest-axe` accessibility checks
- Critical flows have Playwright + `@axe-core/playwright` E2E tests

### Reference

See [ui.accessibility.instructions.md](.github/instructions/ui.accessibility.instructions.md) for complete accessibility patterns and requirements.
