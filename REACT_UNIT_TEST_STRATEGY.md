...
- [ ] **Are my selectors accessible?** (role/label/text, not classes/ids)
- [ ] **Is my Redux provider set up correctly?** (`createMockStoreAndWrapper`)
- [ ] **Are my RTK Query mocks using `vi.hoisted()`?**
- [ ] **Do I have proper async handling?** (`waitFor`, `act`)
- [ ] **Am I clearing mocks between tests?** (`beforeEach` with `vi.clearAllMocks()`)
- [ ] **Are my test names descriptive?** (What should happen, not what the code does)
- [ ] **Do I have error cases?** (Not just happy path)
- [ ] **Are form validations tested?** (Invalid input handling)
- [ ] **Is Redux dispatch tested?** (State changes verified)
...
- **Smart Profit Sharing Test Examples**: `src/pages/DecemberActivities/__test__/`
